...
def main():
    ...
    for pr in candidate_prs:
        ...
        if ci_result.returncode != 0:
            print(f"PR #{pr['number']} CI script failed. Output:\n{ci_result.stdout}")
            conclusion = "failure"
            summary = "CMake build or tests failed when this PR was merged with the current master. Please rebase and ensure all checks pass."
        else:
            print(f"PR #{pr['number']} CI completed successfully.")
            conclusion = "success"
